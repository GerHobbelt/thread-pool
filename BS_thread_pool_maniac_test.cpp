/**
 * @file BS_threadpool_maniac_test.cpp
...
int main()
{
    ...
    dual_println("Here we are looking for race conditions and other failures to keep a consistent state. The goal of this run is to BREAK thread_pool: this might take a while (and hopefully never happens), but we're going to do some nasty things along the way.\n\nYou can run any other applications in parallel with this test, if you believe that will help trigger faults in thread_pool.\n\n");
